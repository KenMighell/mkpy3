...
ORIGINAL CODE: END
    ...
    pass#if
    if (cadence is None): cadence=0  #frame=0
    #
    self = tpf  # alias
    w = self.wcs
...
if (__name__ == '__main__'):
    ...
    print(x0_ra_ll_x0, y0_dec_ll_y0,'=x0_ra_ll_x0, y0_dec_ll_y0 [should be about (0,0)]')
    
    ra_x0_ra_ll_x0, dec_y0_dec_ll_y0 = w.wcs_pix2world(x0_ra_ll_x0, y0_dec_ll_y0, origin0)
    ...
    print()
    frame0 = 0
    # Set one of the next 3 if statements to TRUE depending on the function to be tested
    if (False):
        print('---> check tpf.get_coordinates()')
        rax_ll_x0 = tpf.get_coordinates()[0][frame0][0][0]
        decx_ll_y0 = tpf.get_coordinates()[1][frame0][0][0]
        print('NOTE: next two tests will PASS --- if the tpf.get_coordinates bug has been fixed')
    pass#if
    if (True):
    ...
    if (False):
        ...
        rax_ll_x0 = mkpy3_tpf_get_coordinates_v1(tpf=tpf,recreate_bug=True)[0][frame0][0][0]
        decx_ll_y0 = mkpy3_tpf_get_coordinates_v1(tpf=tpf,recreate_bug=True)[1][frame0][0][0]
    ...
    print(tpf_pos_corr1_frame0, tpf_pos_corr2_frame0,'=tpf_pos_corr1_frame0, tpf_pos_corr2_frame0')

    xx0_rax_ll_x0 = x0_rax_ll_x0 - tpf_pos_corr1_frame0
    yy0_decx_ll_y0 = y0_decx_ll_y0 - tpf_pos_corr2_frame0
    print(xx0_rax_ll_x0, yy0_decx_ll_y0, '=xx0_rax_ll_x0, yy0_decx_ll_y0 [should be about (0,0)]')
    
    ra_xx0_rax_ll_x0, dec_yy0_decx_ll_y0 = w.wcs_pix2world(xx0_rax_ll_x0, yy0_decx_ll_y0, origin0)
