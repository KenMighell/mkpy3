...
# SETI Institute
    ...
    if not skip:
        ...
        if verbose:
            print(
                "%.3f =survey_rotate_deg [deg] : rotation angle "
                "<------------" % survey_rotate_deg
            )
        if is_number:
            ...
            reprojected_survey_data, reprojected_footprint = rp.reproject_interp(
    ...
    while proceed:
        ...
        (
            ...
            ra_deg=ra_deg, dec_deg=dec_deg, radius_arcsec=radius_arcsec, verbose=verbose
        )
        if gaia_dr2_result is None:  # nothing found
            print(gaia_dr2_result, "=gaia_dr2_result [***WARNING*** NOTHING FOUND]")
            break
            # fi
        ...
        if print_gaia_dr2:
            ...
            print()
            print(
                "# GAIA DR2 : Global Astrometric Interferometer for Astrophy"
                "sics-- Data Release 2"
            )
            print(
                "# n GAIA2_Source             sep    RA_ICRS      DE_ICRS   "
                "    pmRA     pmDE      Plx     Gmag"
            )
            print(
                "#                       [arcsec]    [deg]        [deg]     "
            ...
            for k in range(len(xra)):
                ...
                plxj = plx[j]
                print(
                    "%3d %d %8.3f %12.7f %12.7f %8.3f %8.3f %8.3f %8.3f"
            ...
            if sexagesimal:
                print()
                print(
                    "# GAIA DR2 : Global Astrometric Interferometer for Astr"
                    "ophysics -- Data Release 2"
                )
                print(
                    "# n GAIA2_Source          RA_ICRS       DE_ICRS      RA"
                    "_ICRS         DE_ICRS"
                )
                print(
                    "#                         [deg]         [deg]        [h"
    ...
    while proceed:
        ...
        raj2000, dej2000, sep_arcsec, vsx_result = mkpy3.mkpy3_vizier_vsx_cone_get_v2(
            ra_deg=ra_deg, dec_deg=dec_deg, radius_arcsec=radius_arcsec, verbose=verbose
        )
        ...
        vsx_type = np.array(vsx_result["Type"], dtype=str)
        ...
        if print_vsx:
            ...
            if sexagesimal:
                print()
                print("# VSX : AAVSO International Variable Star indeX")
                print("# n   RAJ2000       DEJ2000        RAJ2000        DEJ20" "00")
                ...
                for j in range(raj2000.size):
                    ...
                    ra_ = sc1.ra.to_string(u.hour)
                    dec_ = sc1.dec
                    print("%3d %12.7f %12.7f  %15s %15s" % (k, xraj, ydej, ra_, dec_))
                    # rof
                # fi
            # fi
        break
...
def xmkpy3_tpf_overlay_v6():
    ...
    if obj == 4:
        ...
        print("DOWNLOAD START:", str(datetime.datetime.now()))
        tpf = lk.search_targetpixelfile(
            target=target, mission=mission, quarter=quarter
        ).download(quality_bitmask=0)
        # ^--- exoplanet Kelper-138b is "KIC 7603200"
        print("DOWNLOAD  STOP:", str(datetime.datetime.now()))
        # fi
    ...
    if obj == 5:
        ...
        print("DOWNLOAD START:", str(datetime.datetime.now()))
        tpf = lk.search_targetpixelfile(
            target=target, mission=mission, campaign=campaign
        ).download(quality_bitmask=0)
    ...
    if obj == 6:
        ...
        radius = 120  # arcsec
        print("DOWNLOAD START:", str(datetime.datetime.now()))
        search_results = lk.search_targetpixelfile(
            target, radius=radius, mission=mission, sector=sector
        )
    ...
    mkpy3.mkpy3_plot_add_compass_rose_v5(
        ax=ax, north_arm_arcsec=north_arm_arcsec, wcs=tpf.wcs, verbose=verbose
    )
    ...
    norm = ImageNormalize(
        image_data, interval=PercentileInterval(percentile), stretch=SqrtStretch()
    )
    ax.imshow(image_data, norm=norm, cmap=cmap)
    ax.tick_params(
        axis="x", labelsize=16, length=5, width=2, labeltop=True, labelbottom=True
    )
    ax.tick_params(
        axis="y", labelsize=16, length=5, width=2, labelright=True, labelleft=True
    )
