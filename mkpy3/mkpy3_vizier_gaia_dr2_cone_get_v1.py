...
# NASA Ames Research Center / SETI Institute   
    ...
    if ((ra_deg is None) or (dec_deg is None)): 
        ...
        verbose = True
    pass#if
    if (radius_arcsec is None): radius_arcsec = 300
    #
...
if (__name__ == '__main__'):
    ...
    dec_deg = +42.7843585094725  # RR Lyr
    radius_arcsec = 20
    raj2000, dej2000, sep_arcsec, gaia_dr2_result = mkpy3_vizier_gaia_dr2_cone_get_v1(\
    ...
    print()
    print('# n GAIA2_Source             sep  RA_ICRS       DE_ICRS        pmRA     pmDE      Plx')
    print('#                       [arcsec]  [deg]         [deg]      [mas/yr] [mas/yr]    [mas]')
    ...
    print()
    print('# n GAIA2_Source          Gmag    RA_ICRS       DE_ICRS      RA_ICRS         DE_ICRS')
    print('#                         [mag]   [deg]         [deg]        [hms]           [dms]')
    for k in range(len(xra)):
        j = k #idx[k]
        xraj = xra[j]
        ydej = yde[j]
        ...
        kk = k + 1
        srcj = src[j]
        print('%3d %d %8.3f %12.7f %12.7f  %15s %15s' % (kk,srcj,gmagj,xraj,ydej,ra_,dec_))
